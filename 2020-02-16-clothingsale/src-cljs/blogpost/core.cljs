...
(defn asset-img
  [assetsrc caption]
  [:figure [:a {:href (assets assetsrc)} 
            [:img {:alt caption :src (assets assetsrc) :width "100%"}]]
   [:figcaption caption]])
...
(defn layout []
  [:div {:id "layout"}
   [:div {:style {:display "flex" :flex-direction "row" :flex-wrap "wrap"}}
    ...
    (asset-img "IMG_2288" "01SHAWL-CLAIMED")
    (asset-img "IMG_2292" "02TURTLENECK-MAYBECLAIMED")
    ...
    (asset-img "IMG_2283" "12SCARF-CLAIMED")
    (asset-img "IMG_2282" "13SCARF")
    (let [nomargin {:style {:margin "0"}}]
      [:div#metablock {:style {:width "25vw" :height "25vw" :background-color "black"
                               :position "relative"
                               :top 0}}
       [:div#metablocktext {:style {:word-break "break-all"
                                    :color "white"
                                    :position "absolute"
                                    :bottom "0"}}
        [:p nomargin "PRICE:FREE"]
        [:p nomargin "EMAIL:LH00000000@GMAIL.COM"]]])]])
