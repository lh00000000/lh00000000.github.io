...
if [ -f "index.template.html" ]; then
  ...
  # Extract the built asset paths from dist/index.html
  JS_FILE=$(grep -o 'src="[^"]*\.js"' dist/index.html | head -1 | sed 's/src="//' | sed 's/"//')
  CSS_FILE=$(grep -o 'href="[^"]*\.css"' dist/index.html | head -1 | sed 's/href="//' | sed 's/"//')
  ...
  if [ ! -z "$JS_FILE" ] && [ ! -z "$CSS_FILE" ]; then
    ...
    echo "Warning: Could not extract asset paths from dist/index.html"
  fi
else
  echo "Warning: dist/index.html or index.template.html not found"
fi
